...
function Users() {
  ...
  useEffect(() => {
    fetchCollection('users', usersEndpoint).then(setUsers).catch((error) => setState({ loading: false, error: error.message })).finally(() => setState((current) => ({ ...current, loading: false })))
  }, [usersEndpoint])
